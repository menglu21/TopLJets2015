  ...
  for (Int_t iev=0;iev<nentries;iev++)
    {
     ...
     // if (!passtrigger) continue;
      ...
      std::vector<Particle> leptons_ana = selector.selLeptons(flaggedleptons,muId,SelectionTool::TIGHT,minLeptonPt,2.4);//these are tight leptons fulfill the analysis level selection
      std::vector<Particle> leptons;
      leptons.clear();
      std::vector<Particle> leptons_temp;
      leptons_temp.clear();

      std::vector<Particle> goodMuons = selector.selGoodMuons(flaggedleptons,muId,minLeptonPt,2.4);
      std::vector<Particle> goodEles = selector.selGoodElectrons(flaggedleptons,SelectionTool::TIGHT,minLeptonPt,2.4);
      std::vector<Particle> fakeableMuons = selector.selFakeable_Muons(ev,flaggedleptons,muId,minLeptonPt,2.4);
      std::vector<Particle> fakeableEles = selector.selFakeable_Electrons(flaggedleptons,SelectionTool::VETO, SelectionTool::LOOSE,minLeptonPt,2.4);

      if(leptons.size()>1) continue;//exclude events with at least two true leptons

      sort(goodMuons.begin(),goodMuons.end(),
	            [](const Particle& a, const Particle& b){return a.Pt() > b.Pt();}
      );
      sort(goodEles.begin(),goodEles.end(),
	            [](const Particle& a, const Particle& b){return a.Pt() > b.Pt();}
      );
      sort(fakeableMuons.begin(),fakeableMuons.end(),
	            [](const Particle& a, const Particle& b){return a.Pt() > b.Pt();}
      );
      sort(fakeableEles.begin(),fakeableEles.end(),
	            [](const Particle& a, const Particle& b){return a.Pt() > b.Pt();}
      );

      int fake_tag = -1; //0:one true on fake, 1: two fake
      // with one good muon
      if (goodMuons.size()==1)
      {
	leptons.push_back(goodMuons[0]);
	if (goodEles.size()>0) continue;//this is a duplicate selection
	fake_tag=0;
	if (fakeableMuons.size()>0) 
        {
	  if (fakeableEles.size()>0)
	  {
	    if (fakeableMuons[0].pt()>fakeableEles[0].pt())
	    {
		leptons.push_back(fakeableMuons[0]);
	    }
	    else
		leptons.push_back(fakeableEles[0]);
	  }
	  else
	    leptons.push_back(fakeableMuons[0]);
	}
	else if (fakeableEles.size()>0)
	{
	  leptons.push_back(fakeableEles[0]);
	}
	else
      ...
      // with one good electron
      else if (goodEles.size()==1)
      {
	leptons.push_back(goodEles[0]);
	fake_tag=0;
	if (fakeableMuons.size()>0)
        {
          if (fakeableEles.size()>0)
      ...
      {
	if (fakeableMuons.size()>0){
	  for(auto &mu_fake: fakeableMuons)
	  {leptons_temp.push_back(mu_fake);}
	}
	if (fakeableEles.size()>0){
	  for(auto &ele_fake: fakeableEles)
	  {leptons_temp.push_back(ele_fake);}
	}
	if (leptons_temp.size()<2) continue;
	fake_tag=1;
	sort(leptons_temp.begin(),leptons_temp.end(),
                    [](const Particle& a, const Particle& b){return a.Pt() > b.Pt();}
        );
	leptons.push_back(leptons_temp[0]);
	leptons.push_back(leptons_temp[1]);
      }	
