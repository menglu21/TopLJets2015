...
f = open('signal_tagging.txt','w')

#Read json file
samples = dict()
jsonfile = open(os.path.join(workdir+"/../samples_2017_signal.json"))
...
name = ["_compare_ctag","_compare_ctag_c","_compare_ctag_b","_compare_ctag_udsg"]
histo = []
for i in range(4):
  for j in range(4):
...
Title = ["1.Total","2.MC","3.Loose","4.Medium","5.Tight","6","7","8","9","10"]
    ...
    if ((s in ff) and ("MC" in ff)) :
      ...
      for t in histo:
        ...
        for i in range(Range):
          try:
            subresult[t][i] += h.GetBinContent(i+2)*weight
            result[t][i] += h.GetBinContent(i+2)*weight
          except:
            subresult[t][i] += 0
      inf.Close()
  for t in histo:
